...
describe('executeCommand()', () => {
  ...
  it('calls to callback with parsed args', () => {
    ...
    return Promise.all([
      ...
      expect(pingCallback).toHaveBeenCalledWith({ args: { _: ['ping'], '--': [] } })
      expect(pingCallback).toHaveBeenCalledWith({ args: { _: ['ping', 'you'], '--': [] } })
      expect(meCallback).toHaveBeenCalledWith({ args: { _: ['ping', 'me'], '--': [] } })
      expect(pingCallback).toHaveBeenCalledWith({ args: { _: ['ping'], who: 'me', '--': [] } })
    })
  })
})
